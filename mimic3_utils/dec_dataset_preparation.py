...
import math
import time
from mimic3_utils.mimic_3_files.readers import DecompensationReader
...
# in my case, the dir ends with "mimic3benchmark/scripts/data/decompensation/"
# data_root = 'D:/yaquan/mimic3-benchmarks-master/mimic3benchmark/scripts/data/decompensation/'
data_root = ''
save_dir = os.path.join(pathlib.Path(__file__).parent.absolute(), 'decompensation')
...
train_reader = DecompensationReader(dataset_dir=os.path.join(data_root , 'train'),
                                  listfile=os.path.join(data_root , 'train_listfile.csv'),
                                  n_worker = n_worker)
val_reader = DecompensationReader(dataset_dir=os.path.join(data_root , 'train'),
                                listfile=os.path.join(data_root , 'val_listfile.csv'),
                                n_worker=n_worker)
test_reader = DecompensationReader(dataset_dir=os.path.join(data_root , 'test'),
...
normalizer_state = 'decomp_ts{}.input_str_previous.n1e5.start_time_zero.normalizer'.format(timestep)
normalizer_state = os.path.join(pathlib.Path(__file__).parent.absolute(),
                                'mimic_3_files', 'normalizer', normalizer_state)
normalizer.load_params(normalizer_state)

# Since the data set is too large to be loaded into memory,
# we save and read data by chunks.
batch_size = 256 # each trunk has size 128*batch_size
...
for data_type in data_reader_dict.keys():
    ...
    this_save_dir = os.path.join(save_dir, data_type)
    if not os.path.isdir(this_save_dir):
        os.mkdir(this_save_dir)
    for index, data in enumerate(data_loader_wrapper):
        chunk_sample_size_list.append(len(data[0]))
        this_name = "chunk_" + str(index)
        np.savez_compressed(os.path.join(this_save_dir, this_name), list(data[0]), list(data[1]))
    np.savez_compressed(os.path.join(this_save_dir, 'chunk_sample_sizes'), chunk_sample_size_list)
