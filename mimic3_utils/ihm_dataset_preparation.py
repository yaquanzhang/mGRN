...
import math
import time
from mimic3_utils.mimic_3_files.readers import InHospitalMortalityReader
...
# in my case, the dir ends with "mimic3benchmark/scripts/data/in-hospital-mortality/"
# data_root = 'D:/yaquan/mimic3-benchmarks-master/mimic3benchmark/scripts/data/in-hospital-mortality/'
data_root = ''
save_dir = os.path.join(pathlib.Path(__file__).parent.absolute(), 'in_hospital_mortality')
...
discretizer = Discretizer(timestep=timestep,
                          ...
                          start_time='zero')
train_reader = InHospitalMortalityReader(dataset_dir=os.path.join(data_root, 'train'),
                                         listfile=os.path.join(data_root, 'train_listfile.csv'),
                                         period_length=48.0, )

val_reader = InHospitalMortalityReader(dataset_dir=os.path.join(data_root, 'train'),
                                       listfile=os.path.join(data_root, 'val_listfile.csv'),
                                       period_length=48.0)

test_reader = InHospitalMortalityReader(dataset_dir=os.path.join(data_root, 'test'),
...
normalizer_state = 'ihm_ts{}.input_str_previous.start_time_zero.normalizer'.format(timestep)
