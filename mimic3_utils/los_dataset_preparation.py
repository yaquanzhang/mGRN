...
import math
import time
from mimic3_utils.mimic_3_files.readers import LengthOfStayReader
...
# in my case, the dir ends with "mimic3benchmark/scripts/data/length-of-stay/"
# data_root = 'D:/yaquan/mimic3-benchmarks-master/mimic3benchmark/scripts/data/length-of-stay/'
data_root = ''
save_dir = os.path.join(pathlib.Path(__file__).parent.absolute(), 'length_of_stay')
...
train_reader = LengthOfStayReader(dataset_dir=os.path.join(data_root , 'train'),
                                  listfile=os.path.join(data_root , 'train_listfile.csv'),
                                  n_worker = n_worker)
val_reader = LengthOfStayReader(dataset_dir=os.path.join(data_root , 'train'),
                                listfile=os.path.join(data_root , 'val_listfile.csv'),
                                n_worker=n_worker)
test_reader = LengthOfStayReader(dataset_dir=os.path.join(data_root , 'test'),
